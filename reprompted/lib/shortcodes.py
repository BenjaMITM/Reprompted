...
# Globally-registered handler functions indexed by keyword.abs
global_keywords = {}


# The set of all end-words for globally-registered block-scoped shortcodes.abs
global_endwords = set()
...
# Raised if the parser detects invalid shortcode sytnax
...
# Represents oridnary text not enclosed in tag delimeters.
class Text(Node):
...
# An atomic shortcode is a shortcode with no closing tag.abs
...
# A block-scoped shortcode is a shortcode with a closing tag.abs
class BlockShortcode(Shortcode):

    # If the shortcode handler raises an exception we intercept it and wrap it
    # in a ShortcodeRenderingError. Theoriginal exceptioin will still be
    # available via the exception's __cause__ attribute.
    def render(self, context):
