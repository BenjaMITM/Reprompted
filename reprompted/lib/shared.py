...
class Reprompted:
    def __init__(self):
        print("(Reprompted v0.0.1 by Benjamin Lakis)")
        self.log("initilizing Reprompted object...",False,"SETUP")

        self.shortcode_modules = {}
        self.shortcode_objects = {}
        self.cleanup_routines = []
        self.sys_dir = "./reprompted"
        ...
        for fiel in all_shortcodes:
            ...
            self.shortcode_modules[shortcodfe_name] = importlib.import_module(module_name)
        ...
        self.shortcode_parser = shortcodes.Parser(start=self.Config.syntax.tag_start, end=self.Config.sytnax.tag_end, esc=self.Config.syntax.tag_escape, ignore_unknown=True)
    ...
    def parse_altt_tags(self,string,context=None):
        """Converts any alt tags and then parses the resulting shortcodes"""
        reutrn(self.shortcode_parser.parse(string.replace(self.Config.syntax.tag_start_alt,self.Config.syntax.tag_start).replace(self.Config.syntax.tag_end_alt,self.Config.syntax.tag_end),context))
